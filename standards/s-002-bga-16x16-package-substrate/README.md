# Standard S-002 — BGA 16×16 Package Substrate
...
Defines the normative ball-grid-array layout, row and column addressing, and per-row peripheral assignments for the UMT BGA 16×16 Package Substrate.
...
An implementation conforms to Standard S-002 if and only if:
...
`errata` and `s-002`. Errata that require a normative change trigger
