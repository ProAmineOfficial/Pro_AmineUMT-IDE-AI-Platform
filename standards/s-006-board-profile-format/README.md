# Standard S-006 — Board Profile Format
...
Defines the JSON schema for board profiles under `boards/`. Every board profile must specify MCU, toolchain, flash protocol, pin restrictions, and the VR-to-physical mapping.
...
An implementation conforms to Standard S-006 if and only if:
...
`errata` and `s-006`. Errata that require a normative change trigger
