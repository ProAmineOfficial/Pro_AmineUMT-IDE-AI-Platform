# Standard S-008 — UMT SDK Interface
...
Defines the public C++ API of the UMT SDK — the `UMT.h` master header, the `VRTypes.h` identifier list, and the four lifecycle methods (activate, deactivate, enable, disable) with their state transitions.
...
An implementation conforms to Standard S-008 if and only if:
...
`errata` and `s-008`. Errata that require a normative change trigger
