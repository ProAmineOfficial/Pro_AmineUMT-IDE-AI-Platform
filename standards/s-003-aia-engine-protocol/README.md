# Standard S-003 — AIA Engine Protocol
...
Defines the wire protocol between the front-end IDE and the Abstraction Intelligence Algorithm Engine — the seven-step pipeline, request and response schemas, and error codes.
...
An implementation conforms to Standard S-003 if and only if:
...
`errata` and `s-003`. Errata that require a normative change trigger
