# Standard S-001 — UMT VR API Naming
...
Defines the normative naming conventions for the UMT VR API — interface names (`VR_ADC4`), direction-suffixed pin names (`VR_ADC_IN4`), and the three-level API (`Interface`, `Interface_Pin`, `Digital_Pin`).
...
An implementation conforms to Standard S-001 if and only if:
...
`errata` and `s-001`. Errata that require a normative change trigger
